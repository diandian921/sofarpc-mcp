...
- **resolve 多 server 分支落地选择**:`resolved.Servers` 是 **app `boundServers` 预建的 `[]map{"name","server":Server}`**,tools 拿到的是拼好的 map。倾向 **tools 层自己从 config 重列 server 走 publicServer**(干净,轻微 dup),而非伸手进 app 建好的 map 替 `"server"`(脏、依赖内部 key)。
