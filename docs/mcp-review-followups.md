...
| 项 | 落点 |
|---|---|
| P0 出口脱敏 | 新 `tools/view.go`(`redactAttachments`/`publicServer`/`publicEndpoint`/`publicServers`/`publicPlanDisplay`);接线 resolve.go(单/多)、config.go(list/save_server)、helpers.go(endpointData,doctor 用)、invoke.go(invoke_plan)。测试 `tools/view_test.go`:逐出口 5 个 + sentinel 全扫 `TestNoToolLeaksAttachmentValue` + 注入-store 一致性 + view 单测 |
...
| P2 progressToken 校验 | `proto/progress.go` `validProgressToken`(string + `Int64` 精确整数字面量 ±2^53,见四审终态);测试 `proto/progress_test.go` |
...
### codex 三审(commit c06528e)→ 试 big.Rat,被四审推翻

- **`Int64`-only 被指矫枉过正**:把 `1.0`/`2e3` 这类整数值的合法 JSON 写法也拒了。当时改用 `math/big.Rat` 精确解析以接受所有整数值写法。

### codex 四审(commit 03e4aa0)→ big.Rat 有 DoS,**用户定夺 Int64-only(终态)**

- **big.Rat 招来资源放大**:`1e1000000` 这种 9 字节输入会让 `SetString` 先构造百万位分子再被范围检查拒掉(几十 ms + MB 级)。
- **YAGNI 反推 + 用户拍板**:`progressToken` 是 host 库设的关联 id,**模型/prompt injection 够不着**(非 tool 入参),DoS 仅恶意 host 可触发,本地可信 stdio 下低危;且 `1.0`/`2e3` 现实中**没有 MCP 客户端会这么序列化**。codex 二→三→四审是在为不出现的输入形态追加复杂度。
- **终态(用户选 Int64-only)**:`validProgressToken` 只认 string + `Int64` 能精确解析的 ±2^53 内整数字面量;`1.0`/`2e3`/`1e1000000`/`...e0`/`1.2` 一律拒。3 行、无 `big.Rat`、天然无 DoS、零精度损失。`progress_test.go` 覆盖含 `1e1000000`/`...e0` 边界。**codex 完整性诉求 vs 简洁/鲁棒,取后者——是裁决不是疏漏。**
