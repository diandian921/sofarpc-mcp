...
This matrix tracks the pure-Go direct BOLT/Hessian2 runtime against independent
protocol oracles. Default tests use oracle-bound Java-generated Hessian bytes
and do not require a JVM. The optional Hessian oracle test,
...
The optional BOLT oracle test,
`go test -tags bolt_oracle ./internal/direct`, uses `sofa-bolt-go` as an
independent BOLT frame implementation. It verifies that our request frames are
readable by that implementation, that frames generated by that implementation
are readable by our response reader, and that simplemap header encoding matches
semantically.
...
| `java.time.*` (LocalDate, LocalDateTime, Instant) | Supported | Golden decode + presentation flatten to ISO + Go->Java encode oracle from ISO-string arguments (alipay `jdk8.*Handle` proxies) | Request arguments accept ISO strings (`2024-01-15`, `2024-01-15T10:30:00`, `2024-01-15T10:30:00Z`); covers these three types only. |
