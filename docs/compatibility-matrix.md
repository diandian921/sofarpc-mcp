...
bytes are readable by Java hessian-lite.
...
| `byte[]` | Supported | Golden decode + optional encode oracle | JSON input should be an array of byte numbers in `[-128, 255]`. |
| DTO object | Supported | Golden decode + optional encode oracle | Shared object references are not preserved on request encoding. |
| List with null elements | Supported | Golden decode + optional encode oracle | Set-specific behavior is not separately covered yet. |
| Map | Partial | Golden decode + optional encode oracle with non-string key | Normal flattened results stringify map keys. Use `rawResult=true` for diagnosis. |
