...
不支持（也有意不声明）：`resources`、`roots`、`sampling`、`elicitation`。config 文件可能包含带凭据的 attachments，因此不会作为 resource 暴露。
